...
# Built by lambda-thumbnail/build.sh before `terraform apply` - see the
# milestone 5 doc for the exact command.
resource "aws_lambda_function" "thumbnail" {
  function_name = "tkt-${var.owner_initials}-thumbnail"
  role          = aws_iam_role.lambda_thumbnail.arn
  handler       = "handler.handler"
  runtime       = "python3.12"
  filename      = "${path.module}/../lambda-thumbnail/build/thumbnail.zip"
  source_code_hash = filebase64sha256("${path.module}/../lambda-thumbnail/build/thumbnail.zip")

  timeout      = 15
  memory_size  = 256 # Pillow needs a bit more headroom than the Lambda default 128MB
