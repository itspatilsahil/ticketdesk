...
resource "aws_cloudfront_distribution" "main" {
  ...
  origin {
    ...
    custom_origin_config {
      # ALB only has an HTTP:80 listener today (no ACM cert - that's a
      # stretch goal, see docs/09-stretch-goals.md). CloudFront still
      # serves HTTPS to the browser; only the CloudFront-to-ALB hop
      # inside AWS's own network is plain HTTP.
      http_port              = 80
      https_port              = 443
  ...
  ordered_cache_behavior {
    ...
    cached_methods            = ["GET", "HEAD"]
    cache_policy_id          = "4135ea2d-6df8-44a3-9df3-4b5a84be39ad" # AWS managed "CachingDisabled" - a ticket API response must never be cached
